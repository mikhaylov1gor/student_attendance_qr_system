...
> **Репозиторий монорепа.** Все команды ниже предполагают, что ты находишься
> в `backend/`. Если запускаешь из корня — добавь `cd backend` в начало.
...
### 1. Склонировать и зайти в backend

```bash
git clone <url> kursovaya
cd kursovaya/backend
```
