...
export function LoginPage() {
    ...
    const returnTo = search.get('return_to') || '/sessions';

    // Если уже залогинен — увести сразу.
    useEffect(() => {
        if (principal && (principal.role === 'teacher' || principal.role === 'admin')) {
    ...
    const login = useMutation({
        mutationFn: async (values: { email: string; password: string }) => {
            ...
            // После получения токенов сразу кладём их, чтобы /auth/me пошёл с Bearer.
        ...
        onSuccess: ({ tokens, me }) => {
            if (me.role !== 'teacher' && me.role !== 'admin') {
                ...
                form.setErrors({
                    email: 'Этот кабинет только для преподавателей и администраторов.',
                });
                return;
            }
            setSession({
